	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'white': 
			{
					...
					bg.loadGraphic(Paths.image('adam/bg'));	
			    	add(bg);
			}
			case 'star':
			{
					...
					var bg:FlxSprite = new FlxSprite(-600, -700).loadGraphic(Paths.image('adam/phase_3bg'));
					bg.antialiasing = true;
					bg.scrollFactor.set(0.9, 0.9);
					bg.active = false;
					add(bg);
						
					var stageFront:FlxSprite = new FlxSprite(-650, -900).loadGraphic(Paths.image('adam/mushrooms'));
					stageFront.setGraphicSize(Std.int(stageFront.width * 1.1));
					stageFront.updateHitbox();
		...
		botplayTxt.cameras = [camHUD2];
		timeBar.cameras = [camHUD2];
		timeBarBG.cameras = [camHUD2];
		timeTxt.cameras = [camHUD2];
		...
		if (isStoryMode && !seenCutscene)
		{
			switch (daSong)
			{
				...
				case 'fireworks':
					...
					FlxG.sound.play(Paths.sound('snd_closet_fall'));
					new FlxTimer().start(0.8, function(tmr:FlxTimer)
						{
							camHUD.visible = true;
							remove(blackScreen);
